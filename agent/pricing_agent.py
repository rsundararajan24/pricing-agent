        ...
        async with ClientSession(read, write) as session:
            ...
            while True:
                ...
                if response.stop_reason != "tool_use":
                    # The model didn't ask to call a tool this turn -- most often
                    # this means it just asked YOU something in plain English
                    # (e.g. "do you approve this diff?") and is waiting for a
                    # real reply, not for a tool result. Keep the conversation
                    # going: prompt for a typed reply and loop back around.
                    # (Gated tools like commit_config_change still get their
                    # own hard terminal confirmation below, regardless of what
                    # you say here -- this is just the natural-language side
                    # of the conversation.)
                    user_reply = input("\nYour reply (or 'exit' to end): ").strip()
